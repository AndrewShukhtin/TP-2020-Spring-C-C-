...
TEST(coord_mean_parall, compute_each_coordinate_mean) {
  ...
  exp_coord->data[0] = 4.1191839599609494;
  exp_coord->data[1] = 4.1231742630004860;
  exp_coord->data[2] = 4.1112425994873130;
...
TEST(coord_mean_parall, compute_each_coordinate_mean_invalid_args) {
  if (!compute_each_coordinate_mean_parall(NULL, NULL)) {
    FAIL();
    return;
  }
}

TEST(coord_mean_parall, compute_mean_by_coordinate_invalid_args) {
  const size_t dim = 3;
